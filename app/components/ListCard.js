    ...
    return (
        <div id={'rank' + movie.rank} className=' w-full flex justify-center items-center my-10 px-5'>
            <div id='moviecard' className="flex flex-col tracking-wider items-center border  lg:rounded-lg lg:w-9/12 w-full rounded-2xl shadow md:flex-row lg:hover:scale-110 md:hover:scale-105 hover:scale-100 transition-all ">
                <a href={`https://www.imdb.com/title/${movie.id}/?ref_=adv_li_i`} target='_blank' className='flex lg:flex-row flex-col tracking-wider items-center'>
                    ...
                    <div className="flex flex-col justify-between p-4 leading-normal">
                        ...
                        {extra && <div className=' flex flex-row flex-wrap lg:text-base text-sm'>
                            <h4>Directors:</h4><p className='px-2 flex flex-wrap'>{extra.directors.toString()}</p>
                        </div>}
                        <div className=' flex flex-row flex-wrap  lg:text-base text-sm'>
                            <h4>Genre:</h4><p className='px-2 flex flex-wrap'>{movie.genre.toString()}</p>
                        </div>
                        <div className=' flex flex-row lg:text-base text-sm'>
                            <h4>Rating:</h4><p className='px-2'>{movie.stars}/10{extra ? ` (${extra.reviews})` : ''}</p>
                        </div>
                        <div className=' flex flex-row lg:text-base text-sm'>
                            <h4>Year:</h4><p className=' px-2'>{movie.year}</p>
                        </div>
                    </div>
                </a>
            </div>
